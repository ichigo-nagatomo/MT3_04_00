...
const char kWindowTitle[] = "LD2B_06_ナガトモイチゴ_MT3_04_00";
...
int WINAPI WinMain(HINSTANCE, HINSTANCE, LPSTR, int) {
	...
	while (Novice::ProcessMessage() == 0) {
		...
		if (length != 0.0f) {
			...
			Vec3 restPos = spring.anchor + direction * spring.naturalLength;
			Vec3 displacement = length * (ball.pos - restPos);
			Vec3 restoringForce = -spring.stiffness * displacement;
			Vec3 dampingForce = -spring.dampingCoefficient * ball.velo;
		...
		DrawLine({spring.anchor, ball.pos / 2.0f} , projectionMatrix , viewportMatrix , WHITE);
		DrawSphere({ball.pos, 0.05f} , projectionMatrix , viewportMatrix , ball.color);
